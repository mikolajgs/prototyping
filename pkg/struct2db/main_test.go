...
func getRow() (int64, int64, string, string, string, string, int, int, string, string, string, int64, string, error) {
	...
	var age, price int
	err := dbConn.QueryRow("SELECT * FROM struct2db_test_structs ORDER BY test_struct_id DESC LIMIT 1").Scan(&id, &flags, &primaryEmail, &emailSecondary, &firstName, &lastName, &age, &price, &postCode, &postCode2, &password, &createdByUserID, &key)
...
func getRowById(id int64) (int64, string, string, string, string, int, int, string, string, string, int64, string, error) {
	...
	var primaryEmail, emailSecondary, firstName, lastName, postCode, postCode2, password, key string
	var age, price int
	err := dbConn.QueryRow(fmt.Sprintf("SELECT * FROM struct2db_test_structs WHERE test_struct_id = %d", id)).Scan(&id2, &flags, &primaryEmail, &emailSecondary, &firstName, &lastName, &age, &price, &postCode, &postCode2, &password, &createdByUserID, &key)
...
func getRowCntById(id int64) (int64, error) {
	var cnt int64
	err := dbConn.QueryRow(fmt.Sprintf("SELECT COUNT(*) AS c FROM struct2db_test_structs WHERE test_struct_id = %d", id)).Scan(&cnt)
	return cnt, err
}

func truncateTable() (error) {
	_, err := dbConn.Exec("TRUNCATE TABLE struct2db_test_structs");
	return err
}
